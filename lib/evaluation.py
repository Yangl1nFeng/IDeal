...
def encode_data_merlin(model, data_loader, log_step=10, logging=logger.info):
    ...
    for i, data_i in enumerate(tqdm(data_loader, desc="Processing", unit="batch")):
        # make sure val logger is used
        images, image_lengths, captions, lengths, ids_raw, pos, raw_captions, memory, scene_id, project_img = data_i
        model.logger = val_logger

        # compute the embeddings
        img_emb1, img_emb2, cap_emb1, cap_emb2 = model.forward_emb(images, captions, lengths, image_lengths=image_lengths, pos=pos)
        if img_embs is None:
            img_embs = np.zeros((len(data_loader.dataset), 2 ,img_emb1.size(1)))
            cap_embs = np.zeros((len(data_loader.dataset), 2 ,cap_emb1.size(1)))
        
        memory_list.extend(memory)
        text_list.extend(raw_captions)
        scene_list.extend(scene_id)
        project_list.extend(project_img)

        img_embs[ids_raw, 1, :] = img_emb2.data.cpu().numpy().copy().squeeze()
        cap_embs[ids_raw, 1, :] = cap_emb2.data.cpu().numpy().copy()
    ...
    return img_embs,cap_embs, text_list, memory_list, scene_list, project_list
...
def encode_data(model, data_loader, log_step=10, logging=logger.info):
    ...
    for i, data_i in enumerate(tqdm(data_loader, desc="Processing", unit="batch")):
        # make sure val logger is used
        images, image_lengths, captions, lengths, ids_raw, pos, raw_captions, memory, scene_id = data_i
        model.logger = val_logger
        ...
        # cache embeddings
        # img_embs[ids_raw] = img_emb.data.cpu().numpy().copy()
        # cap_embs[ids_raw, :] = cap_emb.data.cpu().numpy().copy()
    ...
    return img_embs,cap_embs, text_list, memory_list, scene_list
...
def evalrank(model_path, data_path=None, split='dev', fold5=False, save_path=None):
    ...
    data_loader = i_c_bert.get_test_loader(split, opt.data_name, vocab,
                                                opt.batch_size, opt.workers, opt)

    logger.info('Computing results...')
    with torch.no_grad():
        img_embs,cap_embs = encode_data(model, data_loader)
        # img_embs = torch.zeros((img_embs_.shape[0] // opt.hardnum, img_embs_.shape[1])).cuda()
        # cap_embs = torch.zeros((cap_embs_.shape[0] // opt.hardnum, cap_embs_.shape[1])).cuda()
        # for i in range(img_embs.shape[0]):
        #     for j in range(opt.hardnum):
        #         img_embs[i,:] = img_embs[i,:] + img_embs_[opt.hardnum*i+j,:]
        #         cap_embs[i,:] = cap_embs[i,:] + cap_embs_[opt.hardnum*i+j,:]
        # img_embs = torch.div(img_embs, opt.hardnum)
        # cap_embs = torch.div(cap_embs, opt.hardnum)
    ...
    if not fold5:
        ...
        sims_1 = compute_sim(img_embs[:,0,:], cap_embs[:,0,:])
        sims_2 = compute_sim(img_embs[:,1,:], cap_embs[:,1,:])
        # sims_3 = compute_sim(img_embs[:,2,:], cap_embs[:,2,:])
        sims = (sims_1 + sims_2) / 2
...
def i2t(npts, sims, return_ranks=False, mode='coco'):
    ...
    for index in range(npts):
        ...
        if mode == 'coco':
            rank = 1e20
            for i in range(10 * index, 10 * index + 10, 1):
...
def t2i(npts, sims, return_ranks=False, mode='coco'):
    ...
    if mode == 'coco':
        ranks = np.zeros(10 * npts)
        top1 = np.zeros(10 * npts)
    else:
        ranks = np.zeros(npts)
        top1 = np.zeros(npts)
    ...
    for index in range(npts):
        if mode == 'coco':
            for i in range(10):
                inds = np.argsort(sims[10 * index + i])[::-1]
                ranks[10 * index + i] = np.where(inds == index)[0][0]
                top1[10 * index + i] = inds[0]
        else:
            inds = np.argsort(sims[index])[::-1]
